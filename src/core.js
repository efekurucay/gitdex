...
export const OUTPUT_MARKER_FILE = '.gitdex-output';
...
async function ensureCleanOutputDirectory(outputDir, force) {
  ...
  if (!stats) {
    ...
    await fs.writeFile(path.join(outputDir, OUTPUT_MARKER_FILE), 'gitdex output directory\n', 'utf8');
    return;
  }
  ...
  if (!force && !hasMarker && !isEmpty) {
    throw new Error(
      `Output directory already exists and is not a gitdex folder: ${outputDir}. Use --force to clean it or choose --output.`
    );
  }

  await fs.rm(outputDir, { recursive: true, force: true });
  await fs.mkdir(outputDir, { recursive: true });
  await fs.writeFile(markerPath, 'gitdex output directory\n', 'utf8');
}

export async function buildGitdex(inputDir, outputDir, options = {}, hooks = {}) {
